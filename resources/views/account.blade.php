  ...
  <div class="container">
    <div class="row d-flex flex-wrap align-items-center">
      ...
      <div class="header-btns col-sm-8 col-md-6">
        <a href="{{env('APP_NFT_URL')}}dashboard" title="My Collections">My Collections</a>
        <a id="acco" name="acco" href="{{env('APP_NFT_URL')}}account" title="Account">Account</a>
...
-->
    ...
    <div>
      <input type="text" class="form-control" readonly="readonly" value="{{$username}}" size="80" id="username" name="username" required >
    </div>

    <h3 class="form-label">Email<span class="required">*</span></h3>
    <div>
      <input type="text" class="form-control" readonly="readonly" value="{{$email}}" size="80" id="email" name="email" required >
    </div>

    <h3 class="form-label">Password<span class="required">*</span></h3>
    <div>
      <input type="password" class="form-control" value="" size="80" id="pass" name="pass" >
    </div>

    <h3 class="form-label">Name<span class="required">*</span></h3>
    <div>
      <input type="text" class="form-control" value="{{$name}}" size="80" id="name" name="name" required >
    </div>

    <h3 class="form-label">Country<span class="required">*</span></h3>
    <div>
      <input type="text" class="form-control" value="{{$country}}" size="80" id="country" name="country" required >
    </div>
    
    <h3 class="form-label">Sponsor Username</h3>
    <div>
      <input type="text" class="form-control" value="{{$referrer}}" size="80" id="referrer" name="referrer" >
    </div>

    <h3 class="form-label">Mobile Number</h3>
    <div>
      <input type="text" class="form-control" value="{{$mobile}}" size="80" id="mobile" name="mobile" >
    </div>
   ...
   -->
    ...
    @if($successupdate == 'User was updated successfully')
      <div class="container mt-4">
        <div class="alert alert-success container mt-4">
          {{ $successupdate }}
        </div>
    ...
    <div class="form-group container mt-4">  
      <button type="submit" style="float: left;" class="btn btn-success" name="submit" value="Update">Update</button>
      <button type="submit" style="float: right;" onclick="window.location='{{ route('dashboard.page') }}'" name="submit" class="btn btn-success" value="Collections">My Collections</button>
    </div>
...
<script>
  ...
  $("#update").click(function () {
    //$("#inputArea").innerHTML('');
    $("#accountArea").remove(); 
    $("#updateArea").append('<div id="inputArea"></div>');
    $("#updateArea").append('<h2>Upload Images</h2>');
